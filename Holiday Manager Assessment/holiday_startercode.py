...
class Holiday:
    ...
    def asdictionary(self):
        ...
        adict['date'] = self.date.strftime('%Y-%m-%d')
...
class HolidayList:
    ...
    def removeHolidayStr(self, holidayname):
        
        print(self.innerHolidays)
        for i in self.innerHolidays:
            if i.name == holidayname:
                self.innerHolidays.remove(i)
                print("Success!" + i.name + " has been deleted from the holiday list.")
                print(self.innerHolidays)
                return 
            else:
    ...
    def currentweek(self):
        ...
        currentweek = str(week_num)
        
        self.holidaysinweek(self.filterholidays(datetime.today().year, currentweek))

    def filterholidays(self, year, weeknumber):

        holidays = list(filter(lambda x: x.date.isocalendar()[0] == int(year) and x.date.isocalendar()[1] == int(weeknumber), self.innerHolidays))

        return holidays

    def holidaysinweek(self, holidaylist):
...
#         # Use addHoliday function to add holidays to inner list.
    ...
    def save_to_json(self):
        ...
        listofdictionaries = list(map(lambda Holiday: Holiday.asdictionary(), self.innerHolidays))
        print(listofdictionaries)
        jsonfile = json.dumps(listofdictionaries)
        with open('newholidays.json', 'w') as file:
            file.write(jsonfile)
...
def main_menu():
    ...
    if userinput == '1':
        ...
        print("=============")
        holidayremovenameinput = input("Holiday : ")
        hl.removeHolidayStr(holidayremovenameinput)
        #removeaholiday()
    elif userinput == '3':
        hl.save_to_json()
        #main_menu()
        ...
        print("View Holidays")
        print("=============")
        viewyear = input("Which year?: ")
        viewweek = input("Which week? [1-52, leave blank for current week]: ")
        if viewweek == "":
            hl.currentweek()
        else:
            print("These are the holidays for week #" + viewweek + ": ")
            hl.holidaysinweek(hl.filterholidays(viewyear,viewweek))
